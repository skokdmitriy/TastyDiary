...
extension UIFont {
	var weight: Weight {
		guard let wrightNumber = trains[.weight] as? CGFloat
		else {
			return .regular
		}
		return UIFont.Weight(rawValue: wrightNumber)
	}

	var trains: [UIFontDescriptor.TraitKey: Any] {
		(fontDescriptor.object(forKey: .traits) as?
		 [UIFontDescriptor.TraitKey: Any]) ?? [:]
	}
}
